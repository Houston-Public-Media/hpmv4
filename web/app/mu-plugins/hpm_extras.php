...
/**
 * Cron task to pull latest episodes from Classical Classroom
 */
function hpm_cclassroom_update() {
	$cc = get_option( 'hpm_cc_lastupdate' );
	$url = "https://classicalclassroomshow.com/wp-json/wp/v2/posts?per_page=1";
	$remote = wp_remote_get( esc_url_raw( $url ) );
	if ( is_wp_error( $remote ) ) :
		$errors = implode( "\n\n", $remote->get_error_messages() );
		wp_mail( 'jcounts@houstonpublicmedia.org', 'Error downloading Classical Classroom JSON', $errors, [ 'From: HPM CC Bot <webmaster@houstonpublicmedia.org>', 'Content-Type: text/html; charset=UTF-8' ] );
		return false;
	endif;
	$api = wp_remote_retrieve_body( $remote );
	$json = json_decode( $api, TRUE );
	$j = $json[0];
	if ( $j['id'] === $cc['id'] && $j['modified'] === $cc['modified'] ) :
		return false;
	endif;
	$tags = [];
	...
	$link = $j['link'];
	$date = $j['date_gmt'];
	$image = $j['jetpack_featured_media_url'];
	$tax = $j['_links']['wp:term'];
	$tag_url = '';
	foreach ( $tax as $t ) :
	...
	endforeach;
	if ( !empty( $tag_url ) ) :
		$tag_r = wp_remote_get( esc_url_raw( $tag_url ) );
		if ( is_wp_error( $tag_r ) ) :
			$errors = implode( "\n\n", $tag_r->get_error_messages() );
			wp_mail( 'jcounts@houstonpublicmedia.org', 'Error retrieving tags for Classical Classroom episode', $errors, [ 'From: HPM CC Bot <webmaster@houstonpublicmedia.org>', 'Content-Type: text/html; charset=UTF-8' ] );
			return false;
		else :
			$tag_o = wp_remote_retrieve_body( $tag_r );
			$tag_j = json_decode( $tag_o, TRUE );
			foreach ( $tag_j as $tj ) :
				$tags[] = $tj['name'];
			endforeach;
		endif;
	endif;
	$excerpt = str_replace( [ '<p>', '</p>' ], [ '', '' ], $j['excerpt']['rendered'] );
	$content = str_replace( [ '&#8221;', '&#8243;' ], [ '"', '"' ], $j['content']['rendered'] );
	$content = preg_replace(
		[
			'/\[et_pb_image.+src="[a-zA-Z\-_\.0-9\?=\/:]+".+\/\]/',
			'/<!--\[[a-zA-Z0-9 ]+\]>.+<!\[endif\]\-\->/',
			'/(\[[\/a-zA-Z0-9 "_=\.:\(\),#\-]+\])/'
		], [
	...
	preg_match( '/<p>\n?<audio [a-zA-Z=\" 0-9\-%;:]+><source.+src="([a-zA-Z\-_\.0-9\?=\/:]+)".+><a.+>.+<\/a><\/audio><\/p>/', $content, $match );
	if ( !empty( $match ) ) :
		$audio = str_replace( 'http://', 'https://', $match[1] );
		$content = str_replace( $match[0], '[audio mp3="' . $audio . '"][/audio]', $content );
	endif;
	$content = preg_replace( '/<p><\/p>\n/', '', $content ) . '<p>View this article on <a href="' . $link . '">ClassicalClassroomShow.com</a></p>';
	$args = [
		'post_title' => $title,
	...
	if ( is_wp_error( $tmp ) ) :
		...
		wp_mail( 'jcounts@houstonpublicmedia.org', 'Error downloading Classical Classroom image', $errors, [ 'From: HPM CC Bot <webmaster@houstonpublicmedia.org>', 'Content-Type: text/html; charset=UTF-8' ] );
		return false;
	endif;
	$file_array = [
		'name' => basename( $image ),
	...
	if ( is_wp_error( $attach_id ) ) :
		$errors = implode( "\n\n", $attach_id->get_error_messages() );
		wp_mail( 'jcounts@houstonpublicmedia.org', 'Error uploading Classical Classroom attachment', $errors, [ 'From: HPM CC Bot <webmaster@houstonpublicmedia.org>', 'Content-Type: text/html; charset=UTF-8' ] );
		return false;
	endif;
	update_post_meta( $post_id, '_thumbnail_id', $attach_id );
	update_post_meta( $post_id, 'epc_primary_category', 72 );
	update_option( 'hpm_cc_lastupdate', [ 'id' => $j['id'], 'modified' => $j['modified'] ], false);
	wp_mail( 'jcounts@houstonpublicmedia.org', 'Classical Classroom episode successfully uploaded', get_the_permalink( $post_id ) . "\n" . date( 'r' ), [ 'From: HPM CC Bot <webmaster@houstonpublicmedia.org>', 'Content-Type: text/html; charset=UTF-8' ] );
	return true;
}

add_action( 'hpm_cclassroom', 'hpm_cclassroom_update' );
$timestamp = wp_next_scheduled( 'hpm_cclassroom' );
if ( empty( $timestamp ) ) :
	wp_schedule_event( time(), 'daily', 'hpm_cclassroom' );
endif;