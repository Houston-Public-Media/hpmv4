...
class BetterImageCreditsAdmin {
	...
	function __construct( $plugin ) {
		$this->plugin = $plugin;
		add_filter( 'plugin_action_links_better-image-credits/better-image-credits.php', [ $this, 'add_settings_link' ] );
	...
	function add_fields( $form_fields, $post ) {
		...
		if ( preg_match( '|image/.+|', $mime ) ) {
			$form_fields['credits_source'] = $this->get_field(
				$post, 'credits_source', '_wp_attachment_source_name', __( 'Credits', 'better-image-credits' ), __( 'Source name', 'better-image-credits' )
			);
			$form_fields['credits_link'] = $this->get_field(
				$post, 'credits_link', '_wp_attachment_source_url', __( 'Link', 'better-image-credits' ), __( 'Source URL', 'better-image-credits' ), 'widefat', 'url'
			);

			$form_fields['license'] = $this->get_field(
				$post, 'license', '_wp_attachment_license', __( 'License', 'better-image-credits' ), __( 'License type', 'better-image-credits' ), 'widefat license-auto'
			);

			$form_fields['license_link'] = $this->get_field(
				$post, 'license_link', '_wp_attachment_license_url', __( 'License link', 'better-image-credits' ), __( 'License URL', 'better-image-credits' ), 'widefat license-url-auto', 'url'
			);
		}
	...
	function admin_menu() {
		$this->option_hook = add_submenu_page( 'options-general.php', __( 'Image Credits Options', 'better-image-credits' ), __( 'Image Credits', 'better-image-credits' ), 'manage_options', 'image-credits', [ $this, 'options_page' ] );
		add_settings_section( 'default', '', '', 'image-credits' );
		register_setting( 'image-credits', 'better-image-credits-options', [ $this, 'validate_settings' ] );
	...
	function add_settings_field( $id, $title ) {
		add_settings_field( $id, $title, [ $this, 'field_' . $id ], 'image-credits', 'default', [ 'field_name' => "better-image-credits-options[$id]" ] );
	}
	...
	function field_display( $args ) {
		extract( $args ); ?>
		<p><label><input type="checkbox" name="<?php echo $field_name; ?>[]" value="<?php echo IMAGE_CREDIT_BEFORE_CONTENT; ?>" <?php checked( $this->plugin->display_option( IMAGE_CREDIT_BEFORE_CONTENT ) ); ?>><?php _e( 'Before the content', 'better-image-credits' ); ?></label></p>
		<p><label><input type="checkbox" name="<?php echo $field_name; ?>[]" value="<?php echo IMAGE_CREDIT_AFTER_CONTENT; ?>" <?php checked( $this->plugin->display_option( IMAGE_CREDIT_AFTER_CONTENT ) ); ?>><?php _e('After the content', 'better-image-credits'); ?></label></p>
		<p><label><input type="checkbox" name="<?php echo $field_name; ?>[]" value="<?php echo IMAGE_CREDIT_OVERLAY; ?>" <?php checked( $this->plugin->display_option( IMAGE_CREDIT_OVERLAY ) ); ?>><?php _e( 'Overlay on images (results may vary depending on your theme)', 'better-image-credits' ); ?></label></p>
		<p><label><input type="checkbox" name="<?php echo $field_name; ?>[]" value="<?php echo IMAGE_CREDIT_CAPTION; ?>" <?php checked( $this->plugin->display_option( IMAGE_CREDIT_CAPTION ) ); ?>><?php _e( 'Include in captions (images need to be enclosed in [caption] shortcode)', 'better-image-credits' ); ?></label></p>
		<p><label><input type="checkbox" name="<?php echo $field_name; ?>[]" value="<?php echo IMAGE_CREDIT_INCLUDE_HEADER; ?>" <?php checked( $this->plugin->display_option( IMAGE_CREDIT_INCLUDE_HEADER ) ); ?>><?php _e( 'Include credits for header image (support for header image depends on you theme).', 'better-image-credits' ); ?></label></p>
		<p><label><input type="checkbox" name="<?php echo $field_name; ?>[]" value="<?php echo IMAGE_CREDIT_INCLUDE_BACKGROUND;?>" <?php checked( $this->plugin->display_option( IMAGE_CREDIT_INCLUDE_BACKGROUND ) ); ?>><?php _e( 'Include credits for background image (support for background image depends on you theme).', 'better-image-credits' ); ?></label></p>
	...
	function field_template( $args ) {
		extract( $args ); ?>
		<p><input type="text" name="<?php echo $field_name; ?>" class="large-text code"	value="<?php echo esc_attr( IMAGE_CREDITS_TEMPLATE ); ?>" /></p>
	...
	function field_sep( $args ) {
		extract( $args ); ?>
		<p><input type="text" name="<?php echo $field_name; ?>" class="large-text code" value="<?php echo esc_attr( IMAGE_CREDITS_SEP ); ?>" /></p>
	...
	function field_before( $args ) {
		extract( $args ); ?>
		<p><input type="text" name="<?php echo $field_name; ?>" class="large-text code" value="<?php echo esc_attr( IMAGE_CREDITS_BEFORE ); ?>" /></p>
	...
	function field_after( $args ) {
		extract( $args ); ?>
		<p><input type="text" name="<?php echo $field_name; ?>" class="large-text code" value="<?php echo esc_attr( IMAGE_CREDITS_AFTER ); ?>" /></p>
