...
/*
 ...
 $Id: sitemap-ui.php 2706751 2022-04-08 02:56:40Z auctollo $
...
HTML;
		...
		<div class="wrap" id="sm_div">
			<form method="post" action="<?php echo $this->sg->GetBackLink() ?>">
				<h2><?php _e('XML Sitemap Generator for WordPress', 'sitemap');
					echo " " . $this->sg->GetVersion() ?> </h2>
				<?php

				if (get_option('blog_public') != 1) {
				?><div class="error">
						<p><?php echo str_replace("%s", "options-reading.php#blog_public", __('Your site is currently blocking search engines! Visit the <a href="%s">Reading Settings</a> to change this.', 'sitemap')); ?></p>
					</div><?php
						}
								...
								<div class="meta-box-sortabless">
									...
									<?php if (is_super_admin()) : ?>
										...
										<script type="text/javascript">
											...
											var priorities = [0 <?php for ($i = 0.1; $i < 1; $i += 0.1) {
									...
									<ul>
										<li>
											<p><input type="radio" name="sm_b_prio_provider" id="sm_b_prio_provider__0" value="" <?php echo $this->HtmlGetChecked($this->sg->GetOption("b_prio_provider"), "") ?> /> <label for="sm_b_prio_provider__0"><?php _e('Do not use automatic priority calculation', 'sitemap') ?></label><br /><?php _e('All posts will have the same priority which is defined in &quot;Priorities&quot;', 'sitemap') ?></p>
										</li>
										<?php
									...
									?><b><?php _e('Custom taxonomies', 'sitemap') ?>:</b>
											<ul><?php
											...
											if ($this->sg->IsCustomPostTypesSupported()) {
												...
												if (count($custom_post_types) > 0) {
													?><b><?php _e('Custom post types', 'sitemap') ?>:</b>
											<ul><?php
