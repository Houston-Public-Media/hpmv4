...
class IndexProvider extends ProviderCore implements ISitemapProvider {
	...
	private function addRssLatest(): void {
	...
	public function getPage( $type, $page ): array {
		...
		if ( $this->format == "xml" && $globalSettings->newsMode > 0 ) {
			$this->addNews();
		...
		foreach ( $providers as $providerName ) {
			if ( $providerName == "news" || $providerName == "index" ) {
				continue;
			}
			if ( $this->format == "xml" && $providerName == "latest" ) {
				continue;
			}
	...
	private function doPopulate( $provider, $type, $pages ): void {
