...
class PostsProvider extends ProviderCore implements ISitemapProvider {
	public int $maxPageSize = 2500;
	public function getSuppportedTypes(): array {
		$types = [ "page", "post" ];
	return array_merge( $types, self::getPostTypes() );
	}

	public function getPageCount( $type ): float|int {
		if ( $this->exclude( $type ) ) {
			return 0;
		}
	...
	public function getPage( $type, $page ): array {
		if ( $this->exclude( $type ) ) {
			return [];
		}
	...
	private function postTypeDefault( $type ) {
		if ( $type == 'page' ) {
			...
			return ( $this->sitemapDefaults->{$type} ?? $this->sitemapDefaults->posts );
		}
	}
