...
class RssRenderer extends RendererCore implements ISitemapRenderer {
	function renderItem( $url, $isIndex ): void {
		echo '<item>';
		...
		if ( !$isIndex ) {
			echo '<pubDate>' . $this->getDateString( $url->modified, DateTimeInterface::RSS ) . '</pubDate>';
		}
		echo "</item>\n" ;
	}

	function renderIndex( $urls ): void {
		$this->doRender( $urls, true );
	}

	function renderPages( $urls ): void {
		$this->doRender( $urls, false );
	}

	function doRender( $urls, $isIndex ): void {
