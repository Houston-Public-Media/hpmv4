...
*/
	...
	$related_links = $people = $paragraphs = $orgs = $media = $crops = $sound = $keywords = $collection = $members = $multimedia = $keywords_text = [];
	$program = '';
	...
	if ( !empty( $node['audio'] ) ) :
		...
		endforeach;
		$sound = [ 'mp3' => $mp3,'stream' => $mediastream ];
	endif;
	if ( !empty( $node['image'] ) ) :
	...
	$primary_image = $other_media = $other_image = $slidepic = $storylist = $audiolist = $videos = $inserts = [];
	$oi_count = 0;
	$interval = '';
    ...
    $author_info = [ 'name' => 'NPR', 'link' => 'https://npr.org' ];
	...
	$nprdata = [
		'title' => $headline,
		'permalink' => 'https://www.houstonpublicmedia.org/npr/' . date( 'Y/m/d/',$date_offset) . $id . '/'.sanitize_title( $headline ).'/',
		'excerpt' => $subheading,
		'image' => $image,
