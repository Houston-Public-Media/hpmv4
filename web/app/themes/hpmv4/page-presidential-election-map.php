    ...
    <script>
        ...
        const updateStateVotes = (d, Abbr) => {
          ...
          //  console.log("d.properties: "+JSON.stringify(d3.selectAll("path").filter((d) => d.properties.ABBR)));
            let aff = d3.selectAll("path").filter((d) => d.properties.ABBR === Abbr).attr("data-affiliation");
            let stateAb = d3.selectAll("path").filter((d) => d.properties.ABBR === Abbr).attr('data-state');

            d3.selectAll("path").filter((d) => d.properties.ABBR === Abbr).classed(affiliations[aff].class,false);
            aff++;
            if (aff > 2) {
                aff = 0;
            }
            d3.selectAll("path").filter((d) => d.properties.ABBR === Abbr).classed(affiliations[aff].class,true);
            d3.selectAll("path").filter((d) => d.properties.ABBR === Abbr).attr('data-affiliation', aff);
            states[stateAb].affiliation = aff;
            updateVotes();
        };
        ...
        var smallStates = ["11", "25", "44", "09", "34", "10", "24","28", "33"];
        ...
        d3.json("https://hpmwebv2.s3-us-west-2.amazonaws.com/projects/elections/us_states_topo.json", function(error, us) {
            ...
            g.append("g")
                ...
                "transform": function (d) {
                    d3.select(this).attr("data-state", d.properties.ABBR)
                },
            })
                .attr("d", path)
                .attr("stroke", "white")
                .attr("data-affiliation", 0)
                .attr("data-affiliation", 0)
                .attr("class", "states vote-none").on('click', function(d) {
            ...
            g.selectAll("text")
                ...
                .each(function(d) {
                    if(d.properties.ABBR != "PR" && !skippedAbbr.includes(d.properties.ABBR)) {
                        var lines = states[d.properties.ABBR].code.split(' ');
                        ...
                        for (var i = 0; i < lines.length; i++) {
                            d3.select(this).append("tspan")
                                ...
                                .attr("x", function(d) {
                                    ...
                                    var new_x = path.centroid(d)[0] + (parseInt(stateLabelPositions[d.properties.STATE + "x"]) * factor);
                                ...
                                .attr("y", function (d) {
                                    var new_y = path.centroid(d)[1] + parseInt(stateLabelPositions[d.properties.STATE + "y"]);
                                    if (!isNaN(new_y)) {
                                        if (width <= 576) {
                                            new_y -= 2;
                                        }
